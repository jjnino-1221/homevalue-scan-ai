// COMPREHENSIVE V2 CONVERSATION FLOW - WITH UPDATE FUNCTIONALITY
...
function generateComprehensiveResponse(messages) {
  ...
  console.log(`[ENHANCED] Msg #${msgCount}: Step ${session.currentStep} | "${userMessage.substring(0, 40)}" | WaitingForUpdate: ${session.waitingForUpdate}`);

  // PRIORITY: Handle update input
  if (session.waitingForUpdate) {
    const field = session.waitingForUpdate;
    const originalValue = session[field];

    // Store the updated value
    if (field === 'yearBuilt' || field === 'sqft' || field === 'bedrooms' || field === 'bathrooms') {
      const numValue = parseFloat(userMessage);
      if (!isNaN(numValue)) {
        session[field] = numValue;
      }
    } else {
      session[field] = userMessage;
    }

    session.waitingForUpdate = null;

    // Acknowledge update and move to next step
    ...
    const stepHandlers = {
      3: () => ({
        text: `Updated! Now verifying year built.\n\n<strong>Year Built:</strong> ${session.yearBuilt}\n\nIs this correct?`,
        uiPattern: {
          type: 'chips',
          options: [
      ...
      4: () => ({
        text: `Updated! Now verifying square footage.\n\n<strong>Square Footage:</strong> ${session.sqft.toLocaleString()} sq ft\n\nIs this correct?`,
        uiPattern: {
          type: 'chips',
          options: [
      ...
      5: () => ({
        text: `Updated! Now verifying bedrooms.\n\n<strong>Bedrooms:</strong> ${session.bedrooms} bedrooms\n\nIs this correct?`,
        uiPattern: {
          type: 'chips',
          options: [
      ...
      6: () => ({
        text: `Updated! Now verifying bathrooms.\n\n<strong>Bathrooms:</strong> ${session.bathrooms} bathrooms\n\nIs this correct?`,
        uiPattern: {
          type: 'chips',
          options: [
      ...
      7: () => ({
        text: `Updated! Now verifying garage.\n\n<strong>Garage:</strong> ${session.garage}\n\nIs this correct?`,
        uiPattern: {
          type: 'chips',
          options: [
  ...
  if (userMessage.includes('property_type_update')) {
    session.waitingForUpdate = 'propertyType';
    return {
      text: "What is the correct property type?\n\nPlease enter: Single Family, Condo, Townhouse, Multi-Family, or other type",
      toolCalls: [],
      progress: { step: 3, total: 15, title: 'Update Property Type' }
    };
  }
  if (userMessage.includes('property_type_correct')) {
  ...
  // STEP 4: Year Built
  if (userMessage.includes('year_built_update')) {
    session.waitingForUpdate = 'yearBuilt';
    return {
      text: "What year was the property built?\n\nPlease enter the 4-digit year (e.g., 1985)",
      toolCalls: [],
      progress: { step: 4, total: 15, title: 'Update Year Built' }
    };
  }
  if (userMessage.includes('year_built_correct')) {
  ...
  if (userMessage.includes('sqft_update')) {
    session.waitingForUpdate = 'sqft';
    return {
      text: "What is the total square footage?\n\nPlease enter the number (e.g., 2200)",
      toolCalls: [],
      progress: { step: 5, total: 15, title: 'Update Square Footage' }
    };
  }
  if (userMessage.includes('sqft_correct')) {
  ...
  // STEP 6: Bedrooms
  if (userMessage.includes('bedrooms_update')) {
    session.waitingForUpdate = 'bedrooms';
    return {
      text: "How many bedrooms does the property have?\n\nPlease enter the number (e.g., 4)",
      toolCalls: [],
      progress: { step: 6, total: 15, title: 'Update Bedrooms' }
    };
  }
  if (userMessage.includes('bedrooms_correct')) {
  ...
  // STEP 7: Bathrooms
  if (userMessage.includes('bathrooms_update')) {
    session.waitingForUpdate = 'bathrooms';
    return {
      text: "How many bathrooms does the property have?\n\nPlease enter the number (e.g., 2.5 or 3)",
      toolCalls: [],
      progress: { step: 7, total: 15, title: 'Update Bathrooms' }
    };
  }
  if (userMessage.includes('bathrooms_correct')) {
  ...
  // STEP 8: Garage
  if (userMessage.includes('garage_update')) {
    session.waitingForUpdate = 'garage';
    return {
      text: "What type of garage does the property have?\n\nPlease enter: None, 1-car, 2-car, 3-car, or Carport",
      toolCalls: [],
      progress: { step: 8, total: 15, title: 'Update Garage' }
    };
  }
  if (userMessage.includes('garage_correct')) {
